...
class SystemMonitorApp:
    ...
    def create_widget_mode(self):
        # Create a small top-level window for the widget
        self.widget_window = tk.Toplevel(self.root)
        self.widget_window.title("System Monitor Widget")
        self.widget_window.geometry("320x140")
        self.widget_window.attributes("-topmost", True)
        self.widget_window.resizable(False, False)
        self.widget_window.configure(bg=self.current_theme['bg'])

        # Remove window decorations (optional, for a clean widget look)
        ...
        # Make the widget window draggable
        self.widget_window.bind('<Button-1>', self.start_move)
        self.widget_window.bind('<B1-Motion>', self.on_move)

        # Labels for CPU, Memory, Disk, Network
        font_style = ("Segoe UI", 12, "bold")

        self.widget_cpu_label = tk.Label(self.widget_window, text="CPU: 0.0%", bg=self.current_theme['bg'], fg=self.current_theme['fg'], font=font_style)
        self.widget_cpu_label.pack(anchor='w', padx=10, pady=(10,0))

        self.widget_memory_label = tk.Label(self.widget_window, text="Memory: 0.0%", bg=self.current_theme['bg'], fg=self.current_theme['fg'], font=font_style)
        self.widget_memory_label.pack(anchor='w', padx=10)

        self.widget_disk_label = tk.Label(self.widget_window, text="Disk: 0.0%", bg=self.current_theme['bg'], fg=self.current_theme['fg'], font=font_style)
        self.widget_disk_label.pack(anchor='w', padx=10)

        self.widget_network_label = tk.Label(self.widget_window, text="Net: ↑ 0.00 MB/s ↓ 0.00 MB/s", bg=self.current_theme['bg'], fg=self.current_theme['fg'], font=font_style)
        self.widget_network_label.pack(anchor='w', padx=10, pady=(0,10))

        # Start updating the widget data immediately
    ...
    def on_move(self, event):
        """Handle dragging of the widget window"""
    ...
    def update_widget_data(self):
        try:
            if hasattr(self, 'widget_window') and self.widget_window.winfo_exists():
                ...
                # Calculate network speeds (only for widget, don't interfere with main app)
                widget_sent_speed = (current_net_io.bytes_sent - self.last_net_io.bytes_sent) / (1024 * 1024)
                widget_recv_speed = (current_net_io.bytes_recv - self.last_net_io.bytes_recv) / (1024 * 1024)

                self.widget_cpu_label.config(text=f"CPU: {cpu_usage:.1f}%")
                self.widget_memory_label.config(text=f"Memory: {memory_usage:.1f}%")
                self.widget_disk_label.config(text=f"Disk: {disk_usage:.1f}%")
    ...
    def toggle_widget_mode(self):
        if hasattr(self, 'widget_window') and self.widget_window.winfo_exists():
            # Closing widget mode - restore the main window
            self.widget_window.destroy()
            self.root.deiconify()  # Show the main window
            self.root.lift()       # Bring it to front
        else:
            # Opening widget mode - minimize the main window
