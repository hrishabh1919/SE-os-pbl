...
class SystemMonitorApp:
    ...
    def show_process_details(self):
        ...
        try:
            ...
            details = (
                ...
                # Second call gives actual CPU percent
                f"CPU Percent: {proc.cpu_percent(interval=0.2)}%\n"
    ...
    def create_widget_mode(self):
        # Create a sleek, long, and less boxy top-level widget window
        self.widget_window = tk.Toplevel(self.root)
        self.widget_window.title("System Monitor Widget")
        self.widget_window.geometry("650x70")  # Longer and less boxy
        self.widget_window.attributes("-topmost", True)
        self.widget_window.resizable(False, False)
        self.widget_window.configure(bg="black")  # Set background to black

        # Remove window decorations for a clean widget look
        self.widget_window.overrideredirect(True)
    
        # Add a close button (styled to blend in)
        close_btn = tk.Button(
            self.widget_window,
            text="×",
            width=3,
            command=self.toggle_widget_mode,
            bg="#FF5522",                # Bright red background for visibility
            fg="#FFFFFF",                # White text
            borderwidth=1,               # Thin border
            relief="solid",              # Solid border style
            highlightthickness=0,        # No extra highlight border
            font=("Segoe UI", 14, "bold")
        )
        
        close_btn.place(x=600, y=2)  # Adjusted for new width
        ...
        label_fg = "#CCCCCC"  # Subtle, non-distracting text color
